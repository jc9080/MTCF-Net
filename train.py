"""Training entry point for MTCF-Net on the UCI-HAR dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train MTCF-Net on UCI-HAR."
    )
...
def create_loaders(args: argparse.Namespace) -> tuple[Any, Any]:
    """Create the UCI-HAR training and validation/test loaders."""
    ...
    kwargs = supported_kwargs(
        create_uci_har_loaders,
        loader_candidates,
    )

    print(f"DataLoader arguments: {kwargs}")

    loaders = create_uci_har_loaders(**kwargs)
...
def create_model() -> nn.Module:
    """Create the UCI-HAR version of MTCF-Net."""
    ...
    model = mtcf_net_uci_har(**kwargs)

    if not isinstance(model, nn.Module):
        raise TypeError(
            "mtcf_net_uci_har() did not return torch.nn.Module."
        )
...
def main() -> None:
    ...
    print("=" * 60)
    print("MTCF-Net UCI-HAR Training")
    print("=" * 60)
